...
import { SyncOutlined, ThunderboltOutlined, WarningOutlined } from '@ant-design/icons';
...
export default function Dashboard() {
  ...
  const columns = [
    { title: 'ID', key: 'displayId', width: 50, align: 'center' as const, render: (_: unknown, __: BudgetRow, index: number) => index + 1 },
    { title: '主体', dataIndex: 'name', key: 'name', width: 130,
      render: (v: string) => <span style={{ fontWeight: 500 }}>{v}</span> },
    { title: 'SKU', dataIndex: 'sku', key: 'sku', width: 90, render: (v: string | null) => v || '-' },
    { title: '获客助手余额', dataIndex: 'quotaBalance', key: 'quotaBalance', width: 100,
    ...
    { title: '预计剩余消耗天数', dataIndex: 'countdownDays', key: 'countdownDays', width: 130,
      render: (v: number) => {
  ...
  return (
    <div>
      <PageHeader title="资源预警" desc="获客助手余额监控与预计剩余消耗天数预警"
        extra={<Button icon={<SyncOutlined spin={syncing} />} loading={syncing} onClick={handleSyncAll}>同步数据</Button>} />

      <div className="summary-grid">
        <div className="summary-panel" style={{ background: 'linear-gradient(135deg, #e6f7ff 0%, #f0f5ff 100%)', borderColor: '#d6e4ff' }}>
          <Statistic title={<span style={{ fontSize: 13, color: '#595959' }}>今日累计获客助手进量</span>} value={summary.todayConsumption}
            prefix={<ThunderboltOutlined style={{ color: '#1890ff' }} />} valueStyle={{ color: '#cf1322', fontSize: 28, fontWeight: 600 }} />
        </div>
          ...
          {warningList.length > 0 ? (
            warningList.map((r) => (
              <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#ff4d4f', fontWeight: 600, fontSize: 15, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 400, color: '#999' }}>预计剩余消耗天数 {r.countdownDays} 天</span>
              </div>
            ))
          ) : (
      ...
      {warningList.length > 0 && (
        <Alert message={`${warningList.length} 个主体预计剩余消耗天数不足 5 天，请尽快充值`} type="warning" showIcon style={{ marginBottom: 20, borderRadius: 8 }} />
      )}
      ...
      <Table columns={columns} dataSource={data} rowKey="id" loading={loading} pagination={false} size="middle" scroll={{ x: 760 }}
